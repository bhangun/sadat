...
import tech.kayys.wayang.billing.client.ConsumerAccessClient;
...
public class OrganizationService {
    ...
    @Inject
    @RestClient
    ConsumerAccessClient consumerAccessClient;
    ...
    public Uni<Organization> createOrganization(CreateOrganizationRequest request) {
        ...
        return validateOrganizationRequest(request)
                ...
                .flatMap(v -> Panache.withTransaction(() -> {
                    ...
                    org.orgType = request.orgType() != null ? request.orgType() : OrganizationType.BUSINESS;
                    ...
                    org.metadata = request.metadata() != null ? new HashMap<>(request.metadata()) : new HashMap<>();
                ...
                .flatMap(org -> {
                    ...
                    consumerData.put("id", org.tenantId);
                    consumerData.put("tenantId", org.tenantId);
                    consumerData.put("name", org.name);
                    consumerData.put("legalName", org.legalName);
                    consumerData.put("email", org.billingEmail);
                    consumerData.put("billingEmail", org.billingEmail);
                    consumerData.put("address", org.billingAddress != null ? org.billingAddress.toString() : null);
                    consumerData.put("type", org.orgType == OrganizationType.BUSINESS ? "COMPANY" : "INDIVIDUAL");
                    
                    Map<String, Object> casRequest = new HashMap<>();
                    casRequest.put("consumer", consumerData);
                    casRequest.put("taxId", request.taxId());
                    casRequest.put("ktp", request.ktp());
                    
                    try {
                        consumerAccessClient.register(casRequest);
                    } catch (Exception e) {
                        LOG.error("Failed to sync with CAS for org: {}", org.tenantId, e);
                    }
                ...
                .invoke(org -> LOG.info("Organization created: {} ({})", org.name, org.tenantId))
                .onFailure().invoke(error -> LOG.error("Failed to create organization: {}", request.name(), error));
    }
    ...
    public Uni<Organization> updateOrganization(
        ...
        return Panache.withTransaction(() -> Organization.<Organization>findById(organizationId)
                .flatMap(org -> {
                    if (org == null) {
                        return Uni.createFrom().failure(
                ...
                .flatMap(org -> {
                    ...
                    Map<String, Object> casRequest = new HashMap<>();
                    casRequest.put("consumer", consumerData);
                    if (request.taxId() != null) casRequest.put("taxId", request.taxId());
                    if (request.ktp() != null) casRequest.put("ktp", request.ktp());

                    try {
                        consumerAccessClient.register(casRequest);
                    } catch (Exception e) {
                        LOG.error("Failed to sync update with CAS for org: {}", org.tenantId, e);
                    }
                    return Uni.createFrom().item(org);
                })
                .flatMap(org -> auditLogger.logOrganizationUpdated(org)
                        .replaceWith(org))
                .invoke(org -> LOG.info("Organization updated: {}", org.tenantId));
    }
    ...
    public Uni<Organization> suspendOrganization(UUID organizationId, String reason) {
        ...
        return Panache.withTransaction(() -> Organization.<Organization>findById(organizationId)
                ...
                eventPublisher.publishOrganizationSuspended(org, reason)
                        .replaceWith(org))
                .flatMap(org -> auditLogger.logOrganizationSuspended(org, reason)
                        .replaceWith(org));
    }
    ...
    public Uni<Organization> activateOrganization(UUID organizationId) {
        ...
        return Panache.withTransaction(() -> Organization.<Organization>findById(organizationId)
                ...
                .flatMap(org -> notificationService.sendActivationNotification(org)
                        .replaceWith(org))
                .flatMap(org -> eventPublisher.publishOrganizationActivated(org)
                        .replaceWith(org))
                .flatMap(org -> auditLogger.logOrganizationActivated(org)
                        .replaceWith(org));
    }
    ...
    public Uni<Organization> deleteOrganization(UUID organizationId) {
        ...
        return Panache.withTransaction(() -> Organization.<Organization>findById(organizationId)
                ...
                .flatMap(org -> eventPublisher.publishOrganizationDeleted(org)
                        .replaceWith(org))
                .flatMap(org -> auditLogger.logOrganizationDeleted(org)
                        .replaceWith(org));
    }
    ...
    public Uni<List<Organization>> listOrganizations(
        ...
        String query = status != null ? "status = ?1 and deletedAt is null" : "deletedAt is null";

        return status != null ? Organization.<Organization>find(query, status)
                .page(page, size)
                .list()
