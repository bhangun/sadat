...
public class AuditLogger {
    ...
    private Uni<Void> logAudit(
        ...
        return Panache.withTransaction(() -> {
            ...
            return log.persist()
                ...
                .onFailure().invoke(error -> LOG.error("Failed to log audit event", error));
    }
     ...
     */
    public Uni<Void> logSubscriptionCreated(Subscription sub) {
        return logAudit(
                sub.organization.tenantId,
                null,
                AuditActionType.SUBSCRIPTION_CREATED,
                "Subscription",
                sub.subscriptionId.toString(),
                "Subscription created: " + sub.plan.name,
                null,
                Map.of("planId", sub.plan.planId, "billingCycle", sub.billingCycle),
                AuditSeverity.INFO);
    }
     ...
     */
    public Uni<Void> logSubscriptionUpdated(Subscription sub) {
        return logAudit(
                sub.organization.tenantId,
                null,
                AuditActionType.SUBSCRIPTION_UPDATED,
                "Subscription",
                sub.subscriptionId.toString(),
                "Subscription updated",
                null,
                Map.of("planId", sub.plan.planId, "status", sub.status),
                AuditSeverity.INFO);
    }
     ...
     */
    public Uni<Void> logSubscriptionCancelled(Subscription sub, String reason) {
        return logAudit(
                sub.organization.tenantId,
                null,
                AuditActionType.SUBSCRIPTION_CANCELLED,
                "Subscription",
                sub.subscriptionId.toString(),
