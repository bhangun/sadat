...
import tech.kayys.wayang.workflow.service.backup.BackupManager.RestoreOptions;
...
public class BackupResource {
    ...
    public Uni<Response> createFullBackup(BackupRequest request) {
        ...
        if (request != null) {
            options = new BackupOptions(
                request.isEncrypt() != null ? request.isEncrypt() : false,
                request.isCompress() != null ? request.isCompress() : false,
    ...
    public Uni<Response> restoreFromBackup(
        ...
        if (request != null) {
            options = new RestoreOptions(
                request.isSelective() != null ? request.isSelective() : false,
                request.getSelectedItems() != null ? request.getSelectedItems() : java.util.List.of(),
                request.isSkipVerification() != null ? request.isSkipVerification() : false,
        ...
        return backupManager.restoreFromBackup(backupId, options)
                ...
                .onFailure().recoverWithItem(th -> {
                    ...
                    return Response.status(isBackupCorrupted(th) ? Response.Status.CONFLICT : Response.Status.INTERNAL_SERVER_ERROR)
                            .entity(ErrorResponse.builder()
                                    .errorCode(isBackupCorrupted(th) ? "BACKUP_CORRUPTED" : "RESTORE_FAILED")
                                    .message(th.getMessage())
                                    .timestamp(Instant.now())
                                    .build())
                            .build();
    ...
    public Uni<Response> listBackups(
        ...
        BackupFilter.Builder filterBuilder = BackupFilter.builder();
        
        if (type != null) {
            try {
                filterBuilder.type(java.util.Optional.of(tech.kayys.wayang.workflow.model.BackupType.valueOf(type.toUpperCase())));
        ...
        if (from != null) {
            try {
                filterBuilder.from(java.util.Optional.of(Instant.parse(from)));
            } catch (Exception e) {
                return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                        .entity(ErrorResponse.builder()
        ...
        if (to != null) {
            try {
                filterBuilder.to(java.util.Optional.of(Instant.parse(to)));
            } catch (Exception e) {
                return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                        .entity(ErrorResponse.builder()
                                .errorCode("INVALID_DATE_FORMAT")
        ...
        if (status != null) {
            try {
                filterBuilder.status(java.util.Optional.of(BackupManager.BackupStatus.valueOf(status.toUpperCase())));
            } catch (IllegalArgumentException e) {
                return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                        .entity(ErrorResponse.builder()
        ...
        BackupFilter filter = filterBuilder.build();
